	...
	<div class="px-5 pt-5">
		<div class="flex flex-row justify-between border-b border-neutral-400 pb-5">
			...
			{#if data.name}
				<button
					type="submit"
					form="update"
					class="bg-orange-600 px-14 py-2 rounded-md font-semibold text-white hover:bg-orange-700"
					>Save</button
				>
			{/if}
		</div>
		<div class="flex flex-col md:flex-row">
			<form
				...
				action="?/update"
				method="POST"
				class="flex flex-row w-full lg:w-2/3"
			...
			>
				<div class="flex flex-col w-7/12 text-white">
					...
					<input
						...
						class="bg-neutral-800 border border-neutral-500 rounded-md mt-2 p-1"
					/>
			...
			<div class="flex flex-col w-11/12 lg:w-1/3">
				<div class="flex flex-col bg-neutral-800 mt-4">
					...
					<table class="w-full text-white">
						...
						<tr>
							<td class="font-semibold px-5 p-1"> Distance </td>
							<td>
								{data.tot_distance.toFixed(2)} km
							</td>
						</tr>
						<tr>
							<td class="font-semibold px-5 p-1"> Time </td>
							<td>
								{new Date(data.elap_time * 1000).toISOString().substring(11, 19)}
							</td>
						</tr>
						<tr>
							<td class="font-semibold px-5 p-1 pb-5"> Elevation Gain </td>
							<td class="pb-5">
