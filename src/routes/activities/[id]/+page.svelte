	...
	<div class="hidden md:block w-[15%] mr-5">
		<ul class="text-white border border-neutral-500 bg-neutral-800">
			<a href="/activities/{data.id}">
				<li
					class="p-3 pl-5 text-xl border-b border-neutral-500 border-l-4 border-l-orange-500 hover:bg-neutral-900"
				>
					Overview
				</li>
			</a>
					...
					<div class="">
						<lu class="h-full flex text-white list-none">
							{#if data.expand.user.id == user.id}
								...
								{:else}
									<li class="flex items-center justify-center">
										<p class="font-semibold pr-2">Delete ?</p>
									</li>
									<button
										class="w-12 h-full border-s border-neutral-500 hover:text-orange-500 hover:bg-neutral-800"
										on:click={remove}
									>
										<li class="flex items-center justify-center">YES</li>
									</button>
									<button
										class="w-12 h-full border-s border-neutral-500 hover:text-orange-500 hover:bg-neutral-800"
										on:click={() => (confirm = false)}
									>
										<li class="flex items-center justify-center">NO</li>
									</button>
								{/if}
							{/if}
			...
			<div class="flex flex-col md:flex-row">
				<div class="flex flex-col md:w-1/2 border-s border-e border-b border-neutral-500">
					<div class="flex m-5 pb-7 border-b border-neutral-500">
						<div class="w-[20%]">
							<a href="/">
								{#if data.expand.user.avatar}
									<img
										src="{url}/api/files/{data.expand.user.collectionId}/{data.expand.user.id}/{data
											.expand.user.avatar}"
										alt="avatar"
						...
						<div
							class="pl-5 flex flex-col text-white w-[80%]"
					...
					<div class="mx-5 mt-2 mb-5 flex">
						<textarea
							class="bg-neutral-800 border border-neutral-500 rounded-md text-white w-full placeholder-slate-300 placeholder-opacity-50 placeholder:italic"
							rows="1"
							placeholder="Activity Notes"
						></textarea>
						<button class="ms-2 p-1 text-white text-xs border border-neutral-500 rounded-md"
							>Comment</button
						>
					</div>
				</div>
				<div class="flex flex-col md:w-1/2 border-e border-b border-neutral-500">
					<div class="p-5">
						<div class="pb-5">
							<ul class="flex flex-wrap text-white">
								<li class="pr-5">
									<span class="text-3xl">{data.tot_distance.toFixed(2)}</span><span class="text-md"
										>km</span
									><br />
								...
								<li class="pr-5">
									<span class="text-3xl"
										>{new Date(data.elap_time * 1000).toISOString().substring(11, 19)}</span
									><br />
									<span class="text-sm text-neutral-500">Moving Time</span>
								</li>
								<li>
									<span class="text-3xl">{data.tot_elevation * 1000}</span><span class="text-md"
										>m</span
									><br />
							...
							<ul class="flex flex-wrap text-white">
								...
								<li>
									{#if data.tot_calories}
										<span class="text-xl">{data.tot_calories} </span><span class="text-md">kJ</span
										><br />
					...
					<div class="mx-5 mb-5 flex justify-between text-white">
						<div class="w-1/2">GPS</div>
						<div class="w-1/2">Bike: x</div>
					</div>
				</div>
			</div>
		</div>
		...
		{#if data.gpx}
			<div class="mt-5 border border-neutral-500 bg-neutral-800">
				<div class="h-[400px] border-b border-neutral-500">
					<Leaflet view={initialView} zoom={13} {gpx} from="activity" />
				</div>
