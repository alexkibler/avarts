...
<div
	class="min-h-screen bg-neutral-700 flex flex-col md:flex-row {isGamePage ? 'h-screen' : ''}"
...
>
	...
	<div class="flex-1 flex flex-col min-w-0 overflow-hidden">
		...
		<div class="bg-neutral-800 border-b-neutral-600 border-b-2 sticky top-0 shrink-0 md:hidden">
			<Header {data} {isGamePage} />
		</div>

		<main
			class={isGamePage
				? 'flex-1 flex flex-col w-full min-h-0'
				: 'flex-1 overflow-y-auto w-full pb-24 md:pb-8'}
		>
			<slot />
		</main>
	</div>
