...
<nav class="max-w-screen-xl h-24 md:h-12 mx-auto flex justify-between">
  ...
  {#if data.user}
    <div class="flex flex-col md:flex-row overflow-hidden">
      <div class="hidden md:flex items-center px-3 border-neutral-500 border-e-2 h-full group shrink-0">
        <a href="/upload">
      ...
      <div class="md:border-neutral-500 md:border-e-2 group h-1/2 md:h-full border-neutral-500 border-b-2 md:border-b-0 shrink-0">
        <a href ="/athlete" class="pl-4 pr-2 flex items-center h-full">
          {#if data.user.avatar}
            <img src="{url}/api/files/{data.user.collectionId}/{data.user.id}/{data.user.avatar}" alt="avatar" class="h-8 w-8 object-cover mr-2 rounded-full overflow-hidden shrink-0">
          {:else}
            <img src="/avatar.svg" alt="avatar" class="h-8 w-8 object-cover mr-2 rounded-full overflow-hidden shrink-0">
          {/if}
          <svg class="text-neutral-500 group-hover:text-orange-500 transition-colors duration-300 hidden md:block shrink-0" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
        </a>
      </div>
      {#if !isGamePage}
      <form action="/logout" method="POST" class="flex px-4 h-1/2 md:h-full pl-5 md:pl-4 shrink-0">
