  ...
  {:else}
    <div class="space-y-4">
      {#each sessions as session}
        <div class="bg-neutral-800 rounded-lg border border-neutral-700 p-5 flex flex-col sm:flex-row sm:items-center gap-4">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              <h2 class="text-white font-semibold text-lg truncate">{session.ap_seed_name}</h2>
              <span class="text-xs font-medium px-2 py-0.5 rounded-full {session.status === 'Active' ? 'bg-green-900 text-green-300' : 'bg-neutral-700 text-neutral-400'}">
                {session.status}
              </span>
            </div>
          ...
          <div class="flex gap-2 shrink-0">
            {#if session.status === 'Active'}
              ...
              <button on:click={() => markCompleted(session.id)}
