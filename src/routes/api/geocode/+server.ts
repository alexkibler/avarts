...
export const GET: RequestHandler = async ({ url }) => {
	...
	async function fetchHere(): Promise<{ hits: any[] } | null> {
		...
		try {
			const hereUrl = `https://geocode.search.hereapi.com/v1/geocode?q=${encodeURIComponent(
				q!
			)}&limit=${limit}&lang=${locale}&apiKey=${hereKey}`;
			...
			const hits = (data.items || []).map((item: any) => {
				...
				return {
					...
					extent: item.mapView
						? [item.mapView.west, item.mapView.south, item.mapView.east, item.mapView.north]
						: undefined
				};
			});
	...
	async function fetchNominatim(): Promise<{ hits: any[] }> {
		...
		try {
			const nominatimUrl = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(
				q!
			)}&format=json&limit=${limit}&accept-language=${locale}&addressdetails=1`;
	...
	if (graphApi) {
		try {
			const cloudUrl = `https://graphhopper.com/api/1/geocode?q=${encodeURIComponent(
				q
			)}&key=${graphApi}&limit=${limit}&locale=${locale}`;
