<script lang="ts">
	...
	function calculateMaxDistance(records: Exercises, activityType: string) {
		...
		return filteredRecords.length > 0
			? filteredRecords.reduce(
					(max, record) => (record.tot_distance > max.tot_distance ? record : max),
					filteredRecords[0]
				)
			: { tot_distance: 0 };
	}
	...
	function calculateMaxElevation(records: Exercises, activityType: string) {
		...
		return filteredRecords.length > 0
			? filteredRecords.reduce(
					(max, record) => (record.tot_elevation > max.tot_elevation ? record : max),
					filteredRecords[0]
				)
			: { tot_elevation: 0 };
	}
...
<div class="mt-12 bg-neutral-800 sticky top-32 left-0">
	<h3 class="pl-6 pt-8 text-xl text-white font-semibold">My stats</h3>
	<div class="px-5 pt-6 pb-3 border-b border-neutral-500 w-full">
		<ul class="flex flex-wrap text-white w-fit border bg-neutral-800 rounded-md overflow-hidden">
			<button class="flex" on:click={() => (activityType = 'cycling')}>
				<li
					class="p-2 border-e border-neutral-500 {activityType != 'cycling'
			...
			<button class="flex" on:click={() => (activityType = 'running')}>
				<li
					class="p-2 border-e border-neutral-500 {activityType != 'running'
						? 'hover:text-orange-500'
						: 'bg-neutral-900 text-orange-500'}"
				>
					<svg
						...
						/><path d="M16 17h4" /><path d="M4 13h4" /></svg
					>
				</li>
			</button>
			<button class="flex" on:click={() => (activityType = 'swimming')}>
				<li
					class="p-2 {activityType != 'swimming'
						? 'hover:text-orange-500'
						: 'bg-neutral-900 text-orange-500'}"
				>
					<svg
		...
		<table class="w-full text-white text-sm table-fixed">
			<tbody class="leading-7">
				<tr class="border-b border-neutral-500 leading-10">
					<th class="text-start w-2/3"> Last 4 Weeks </th>
				</tr>
				...
				<tr class="border-b border-neutral-500">
					...
					<td>
						{(
							month
								.filter((item) => item.sport === activityType)
								.reduce((sum, month) => sum + month.tot_distance, 0) / 4
						).toFixed(2)} km
					</td>
				</tr>
				<tr class="border-b border-neutral-500">
					...
					<td>
						{(
							(month
								.filter((item) => item.sport === activityType)
								.reduce((sum, month) => sum + month.tot_elevation, 0) *
								1000) /
							4
						).toFixed(2)} m
					</td>
				...
				<tr class="border-b border-neutral-500">
					...
					<td>
						{formatSumTime(
							month
								.filter((item) => item.sport === activityType)
								.reduce((sum, month) => sum + month.tot_time, 0)
						)}
					</td>
				</tr>
			</tbody>
			<tbody class="leading-7">
				...
				<tr class="border-b border-neutral-500">
					...
					<td>
						{year
							.filter((item) => item.sport === activityType)
							.reduce((sum, year) => sum + year.tot_distance, 0)
							.toFixed(2)} km
					</td>
				</tr>
				<tr class="border-b border-neutral-500">
					...
					<td>
						{(
							year
								.filter((item) => item.sport === activityType)
								.reduce((sum, year) => sum + year.tot_elevation, 0) * 1000
						).toFixed(2)} km
					</td>
				</tr>
				<tr class="border-b border-neutral-500">
					...
					<td>
						{formatSumTime(
							year
								.filter((item) => item.sport === activityType)
								.reduce((sum, year) => sum + year.tot_time, 0)
						)}
					</td>
				</tr>
			</tbody>
			<tbody class="leading-7">
				<tr class="border-b border-neutral-500 leading-10 align-bottom">
					<th class="text-start"> All-Time </th>
				</tr>
				<tr class="border-b border-neutral-500">
					<td> Activities </td>
					<td>
				...
				<tr class="border-b border-neutral-500">
					...
					<td>
						{records
							.filter((item) => item.sport === activityType)
							.reduce((sum, records) => sum + records.tot_distance, 0)
							.toFixed(2)} km
					</td>
				</tr>
				<tr class="border-b border-neutral-500">
					...
					<td>
						{(
							records
								.filter((item) => item.sport === activityType)
								.reduce((sum, records) => sum + records.tot_elevation, 0) * 1000
						).toFixed(2)} m
					</td>
				</tr>
				<tr class="border-b border-neutral-500">
					...
					<td>
						{formatSumTime(
							records
								.filter((item) => item.sport === activityType)
								.reduce((sum, records) => sum + records.tot_time, 0)
						)}
					</td>
				</tr>
				<tr class="border-b border-neutral-500">
					<td>
						Longest {#if activityType == 'cycling'}<span>Ride</span
							>{:else if activityType == 'running'}<span>Run</span
							>{:else if activityType == 'swimming'}<span>Swim</span>{/if}
					</td>
					<td>
