	...
	<div class="mt-12 bg-neutral-800">
		<a href="/" class="pointer-events-none">
			{#if data.user.avatar}
				<img
					...
					alt="avatar"
					class="h-24 w-24 object-cover rounded-full mx-auto -m-12 pointer-events-auto"
				/>
			{:else}
				<img
					src="/avatar.svg"
			...
			<ul class="flex flex-wrap mt-5">
				<a href="/" class="w-1/3 mx-auto text-center">
					<li class="flex flex-col border-e border-neutral-500 group">
						...
						<span class="text-xl text-white group-hover:text-orange-500"> 0 </span>
					</li>
				</a>
		...
		<div class="flex flex-col mt-8 px-4 text-white">
			...
			<div class="pb-5 border-b border-neutral-500">
				{#if records.length > 0}
					<span class="text-md font-semibold hover:text-orange-500"
						><a href="/activities/{records[0].id}"
							>{records[0].name}
							<span class="text-sm font-normal"
								>- {new Date(records[0].start_time).toISOString().split('T')[0]}</span
							></a
						></span
					>
				{/if}
