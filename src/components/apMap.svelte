					...
					.then((data) => {
						const results = (data.hits || []).map((hit: any) => {
							return {
								...
								bbox: L.latLngBounds(
									L.latLng(hit.point.lat, hit.point.lng),
									L.latLng(hit.point.lat, hit.point.lng)
								),
								properties: hit
							};
		...
		navigator.geolocation.getCurrentPosition(
			(position) => {
				...
				if (!myLocationMarker) {
					...
					myLocationMarker.bindTooltip('My Location (click to route here)', {
						direction: 'top',
						offset: [0, -6]
					});
			...
			(err) => {
				...
				if (err.code === 1)
					msg = 'Location permission denied. Please enable location access for this site.';
				else if (err.code === 2)
					msg = 'Position unavailable. Your device could not determine your location.';
		...
		if (activeWaypointIds.has(node.id)) {
			...
			if (routingControl) {
				const wps = routingControl.getWaypoints().filter((w: any) => {
					...
					const isMatch =
						Math.abs(node.lat - w.latLng.lat) < 0.0001 &&
						Math.abs(node.lon - w.latLng.lng) < 0.0001;
					return !isMatch;
				});
				routingControl.setWaypoints(wps);
			}
...
<div class="mockup-app-root {isTestMode ? 'playwright-test' : ''}">
				...
				<div class="panel-body">
					...
					{:else}
						<button
							...
							class="btn-route-available"
							style="margin-bottom: 8px;">Route To Available</button
						>
							...
							{#each accordionStates as state}
								<div class="accordion">
									<button
										...
										on:click={() =>
											(expandedAccordion = expandedAccordion === state ? null : state)}
									>
									...
									{#if expandedAccordion === state}
										<div class="accordion-body">
											{#each nodes.filter((n) => n.state === state) as node}
												...
												<div
													class="node-item"
													style="cursor: {state === 'Hidden'
														? 'default'
														: 'pointer'}; opacity: {state === 'Hidden'
														? 0.5
														: 1}; background: {activeWaypointIds.has(node.id)
														? 'var(--orange-dim)'
														: ''};"
												>
													<div
														style="display: flex; align-items: center; flex: 1; overflow: hidden;"
														on:click={() => handleNodeTap(node)}
													>
														<span
	...
	<div class="route-stats">
		...
		<div class="bottom-row-mobile" style="display: contents;">
			...
			<button
				on:click={exportToGPX}
				disabled={!route}
				class="btn-export mobile-hud-export"
				style="opacity: {route ? 1 : 0.4}; cursor: {route ? 'pointer' : 'not-allowed'}"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					width="14"
...
<style>
	...
	* {
	...
	.map-container {
		position: absolute;
		inset: 0;
		background: linear-gradient(135deg, #2d3a2d 0%, #1f2b1f 30%, #263026 60%, #1a241a 100%);
		overflow: hidden;
	}
	/* Fake map texture */
	...
	.route-instructions strong.orange-pin {
		color: var(--orange);
	}
	.route-instructions strong.green-pin {
		color: var(--green);
	}
	...
	:global(
			.leaflet-routing-geocoder
				> :not(input):not(.leaflet-routing-remove-waypoint):not(
