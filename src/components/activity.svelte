		...
		<div class="flex">
				...
				<a href="/">
					{#if user.avatar}
						<img
							...
							alt="avatar"
							class="h-16 w-16 object-cover rounded-full aspect-square"
						/>
					{:else}
			...
			<div class="flex flex-col text-white pt-2 md:w-[85%]">
				...
				<span class="flex flex-col md:flex-row">
					...
					<span
						>{#if location}<span class="hidden md:inline">&nbsp· </span>{location}{/if}</span
					>
				</span>
			</div>
		</div>
			...
			<ul class="flex flex-wrap mt-5">
				<li class="flex flex-col w-1/2 md:w-1/4 pl-4 border-e border-neutral-500">
					...
					<span class="text-xl text-white"
						>{distance.toFixed(2)}<span class="text-sm">&nbspkm</span></span
					>
				</li>
				...
				<li class="hidden md:flex flex-col w-1/4 pl-4">
					...
					<span class="text-xl text-white"
						>{new Date(time * 1000).toISOString().substring(11, 19)}</span
					>
				</li>
			</ul>
		</div>
			...
			<div>
				<a href="/activities/{id}">
					<div class="w-full h-[300px] overflow-hidden">
						<img
							src="{url}/api/files/{collectionId}/{id}/{img}"
							alt="Activity Route"
