...
export class MockAuthStore {
  ...
  exportToCookie(options: any) { return 'mock_pb_auth=mock_token; Path=/; SameSite=Lax'; }
  clear() {
    this.isValid = false;
    this.model = null;
  }
...
export class MockPocketBase {
  ...
  files = {
    getUrl: (record: any, filename: string) => `http://mock.url/files/${record.collectionId}/${record.id}/${filename}`
  };
